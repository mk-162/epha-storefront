...
export function NewsletterForm() {
  ...
  if (state.successMessage) {
    return (
      <div className="animate-fade-in flex flex-col items-start gap-2 rounded border border-accent/20 bg-accent/5 p-4">
        <p className="text-sm font-bold uppercase tracking-wider text-accent">Success!</p>
      ...
      <form {...getFormProps(form)} action={formAction} className="flex gap-2">
        <div className="relative flex-1">
          <input
            ...
            className="w-full rounded border border-white/10 bg-white/5 px-3 py-2 text-sm text-white transition-colors focus:border-accent focus:outline-none"
            placeholder="Email Address"
          />
        </div>
        <button
          className="flex min-w-[60px] items-center justify-center rounded bg-accent px-4 py-2 text-sm font-bold uppercase text-white transition-all hover:bg-accent/90 active:scale-95 disabled:opacity-50"
          disabled={isPending}
          type="submit"
        >
